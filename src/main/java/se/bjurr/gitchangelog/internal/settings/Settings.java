...
public class Settings implements Serializable {
	...
	/**
	 * A date that is evaluated on the commit time of each commit. If the commit is
	 * older than the point in time given, then it will be filtered out and not
	 * included in the changelog. <br>
	 ...
	 * {@link Changelog#getTags()}, that includes those commits. A fitting value may
	 * be "Next release".
	 */
	...
	private boolean removeIssueFromMessage;
	 /**
	 * URL pointing at your JIRA server. When configured, the {@link
	 Issue#getTitle()} will be
	 ...
	 * Pattern to recognize JIRA:s.
	 <code>\b[a-zA-Z]([a-zA-Z]+)-([0-9]+)\b</code><br>
	 * <br>
	...
	/**
	 * Commits that don't have any issue in their commit message will not be
	 * included.
	 */
	...
	private Map<SettingsIssueType, IssueIntegration> issuesIntegration = new HashMap<>();

	private class IssueIntegration {
		...
		/** Pattern to recognize Issues. <code>#([0-9]+)</code> */
		private Optional<String> issuePattern;
		...
		public Optional<String> getIssuePattern() {
			return issuePattern;
		}

		public void setIssuePattern(String issuePattern) {
			this.issuePattern =  Optional.of(issuePattern);
		}
	...
	public String getGitLabIssuePattern() {
		return (getIssuePattern(GITLAB)).or(DEFAULT_GITLAB_ISSUE_PATTERN);
	}

	public Optional<String> getGitLabProjectName() {
		return getProjectName(GITLAB);
	}

	public Optional<String> getServer(SettingsIssueType tracker) {
		return fromNullable(getIssueIntegration(tracker).getServer());
	}

	public Optional<String> getToken(SettingsIssueType tracker) {
		return fromNullable(getIssueIntegration(tracker).getToken());
	}

	public Optional<String> getIssuePattern(SettingsIssueType tracker) {
		return (getIssueIntegration(tracker).getIssuePattern());
	}

	public Optional<String> getProjectName(SettingsIssueType tracker) {
		return fromNullable(getIssueIntegration(tracker).getProjectName());
	}

	public Optional<String> getApi(SettingsIssueType tracker) {
		return fromNullable(getIssueIntegration(tracker).getApi());
	}

	private IssueIntegration getIssueIntegration(SettingsIssueType tracker) {

		if (!issuesIntegration.containsKey(tracker)) {
			IssueIntegration integration = new IssueIntegration();
			issuesIntegration.put(tracker, integration);
		}
		return issuesIntegration.get(tracker);
	}


	public void setIssuePattern(SettingsIssueType tracker, String gitLabIssuePattern2) {
		getIssueIntegration(tracker).setIssuePattern(gitLabIssuePattern2);

	}

	public void setProjectName(SettingsIssueType tracker, String gitLabProjectName2) {
		getIssueIntegration(tracker).setProjectName(gitLabProjectName2);

	}

	public void setServer(SettingsIssueType tracker, String gitLabServer2) {
		getIssueIntegration(tracker).setServer(gitLabServer2);

	}

	public void setToken(SettingsIssueType tracker, String token) {
		getIssueIntegration(tracker).setToken(token);
