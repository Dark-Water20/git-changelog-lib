...
public class IssueParser {
	...
	public List<ParsedIssue> parseForIssues(final boolean useIntegrationIfConfigured) {
		...
		final RedmineClient redmimeClient = useIntegrationIfConfigured ? createRedmineClient() : null;
		...
		for (final GitCommit gitCommit : commits) {
			...
			for (final SettingsIssue issuePattern : patterns) {
				final Matcher issueMatcher = compile(issuePattern.getPattern()).matcher(gitCommit.getMessage());
				while (issueMatcher.find()) {
					...
					if (!parsedIssuePerIssue.containsKey(matchedIssue)) {
						...
						if (issuePattern.getType() == SettingsIssueType.REDMINE) {
							final String projectName = settings.getProjectName(SettingsIssueType.REDMINE).get();
							parsedIssue = createParsedIssue(redmimeClient,projectName, issuePattern, matchedIssue);
						} else {
			...
			if (!commitMappedToAtLeastOneIssue && !settings.ignoreCommitsWithoutIssue()) {
				...
				final ParsedIssue noIssue = new ParsedIssue(NOISSUE, settings.getNoIssueName(), issue, "", link, title,
						issueType, linkedIssues, labels);
	...
	private ParsedIssue createParsedIssue(final GitLabClient gitLabClient, final String projectName,
			final SettingsIssue issuePattern, String matchedIssueString) {
		String link = "";
		String title = "";
	...
	private ParsedIssue createParsedIssue(final RedmineClient redmineClient, final String projectName,
			final SettingsIssue issuePattern, String matchedIssueString) {
		String link = "";
		String title = "";
		String desc = "";
		...
		final Integer matchedIssue = Integer.parseInt(matchedIssueString);
		try {
			if (redmineClient != null && redmineClient.getIssue(projectName, matchedIssue).isPresent()) {
				final RedmineIssue redmineIssue = redmineClient.getIssue(projectName, matchedIssue).get();
				link = redmineIssue.getLink();
				title = redmineIssue.getTitle();
				desc = redmineIssue.getDescription();
				linkedIssues = redmineIssue.getLinkedIssues();
			}
		} catch (final GitChangelogIntegrationException e) {
			LOG.error(matchedIssueString, e);
		}
	...
	private RedmineClient createRedmineClient() {
		...
		if (settings.getServer(SettingsIssueType.REDMINE).isPresent()) {
			final String server = settings.getServer(SettingsIssueType.REDMINE).get();
			final String token = settings.getToken(SettingsIssueType.REDMINE).orNull();
			client = new RedmineClient(server, token);
		}
		return client;
	}
	...
	private ParsedIssue createParsedIssue(final SettingsIssue issuePattern, final Matcher issueMatcher,
	...
	private ParsedIssue createParsedIssue(final JiraClient jiraClient, final SettingsIssue issuePattern,
			final String matchedIssue) {
		String link = "";
		String title = "";
		String desc = "";
	...
	private ParsedIssue createParsedIssue(final GitHubHelper gitHubHelper, final SettingsIssue issuePattern,
			final String matchedIssue) {
		String link = "";
		String title = "";
